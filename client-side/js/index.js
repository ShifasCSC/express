    ...
    data.map((user)=>{
        ...
        str+=`<div class="card" >
            <div class="images" >
                ...
                <h3>${user.username}</h3>
                <h4>${user.email}</h4>
               <a href="./pages/edit.html?id=${user._id}"> <button type="button" class="btn btn-warning" >EDIT</button></a>
                <button type="button" class="btn btn-dark" onclick="deleteUser('${user._id}')">DELETE</button>
            </div>
        </div>`
    })
